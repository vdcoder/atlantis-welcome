...
using Atlantis.Api.World.Geometry.Utils;

namespace Atlantis.Api.World.Geometry
{
    public class TextSpatialObject
    {
    ...
    public class TextSpatial
    {
        public List<string> GetVisibleAsText(BasicCamera a_camera, Octree a_octree)
        {
            List<string> visibleTextObjects = new List<string>();
            List<TextSpatialObject> visibleTextSpatialObjects = GetVisible(a_camera, a_octree);

            foreach (var TextSpatialObject in visibleTextSpatialObjects)
            {
                // Formats as: "A red building - 45.20m to your front-left and slightly above"
                visibleTextObjects.Add($"{TextSpatialObject.FuzzyDescription} - {TextSpatialObject.FuzzyDistance} {TextSpatialObject.FuzzyDirection}");
            }
            return visibleTextObjects;
        }

        public List<TextSpatialObject> GetVisible(BasicCamera a_camera, Octree a_octree)
        {
            List<TextSpatialObject> visibleTextObjects = new List<TextSpatialObject>();
            ...
            Frustum frustrum = new Frustum(a_camera, nFOVRadians, nAspectRatio, nNearPlane, nFarPlane);
            ...
            foreach (var octreeObject in visibleOctreeObjects)
            {
                ...
                visibleTextObjects.Add(new TextSpatialObject
                {
